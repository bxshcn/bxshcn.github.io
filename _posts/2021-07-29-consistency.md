...
分布式的本质是并发，如果没有并发，那么分布式就没有意义。但一个并发的有效的系统需要考虑很多问题，但最基本的问题可以归结为两点：
...
显然，linearizablity比sequential consistency约束更强，通过将正确性直接构建在object上，并引入real-time （从而支持将任意操作行为原子化），linearizability不仅保留了单进程上操作的先后序列，还保留了多进程之间在共享数据操作上的先后序列。

**很多情况下，一个系统有多种场景，每种场景的正确性要求不同，因此我们实际上会实现多种一致性模型，比如有时候我们在编程时遵循sequential consistency即可，但对于共享数据的并发操作的正确性，我们往往依赖于CAS以及相关的锁机制，即要遵循于linearizability。**
...
### serializability
serializability是一个比较特殊的正确性条件，它主要应用在数据库领域，与事务transaction和隔离性isolation相关。我们通常所说的都是strict serializability。
